...
$(cp $OUTPUT_TEMPLATE_JSON $CURRENT_OUTPUT_FILE_RT)

function join { local IFS="$1"; shift; echo "$*"; } #array to comma-separated string

# Replace checksum: 
sed -i 's/"checksum" : "checksum"/"checksum" : "'$INPUT_CHECKSUM'"/g' $CURRENT_OUTPUT_FILE_RT
...
do    
    ...
    if [ "${INTENSITIES_CHECK}" = "[]" ]; then
        ...
        RT_MAX=`echo $RT_MAX*60 | bc`
        sed -i 's/"value" : "'$HEAVY_COMPLETE_SEQUENCE'ZERO"/"value" : "'$RT_MAX'"/g' $CURRENT_OUTPUT_FILE_RT #RT output file
        ...
        do
            ...
            RT_LIGHT_MAX=`echo $RT_LIGHT_MAX*60 | bc`
            sed -i 's/"value" : "'$LIGHT_COMPLETE_SEQUENCE'ZERO"/"value" : "'$RT_LIGHT_MAX'"/g' $CURRENT_OUTPUT_FILE_RT #RT output file
        done
    fi
done

# Undetected isotopologues to value=0: 
sed -i 's/"value" : "[^ ]*ZERO"/"value" : "0"/g' $CURRENT_OUTPUT_FILE_RT #RT output file
